...
const (
	...
	BindingEngineRevision = "forcefield-binding-engine/v1"
)
...
func compileServices(compiled *Compiled) error {
	...
	for _, name := range sortedKeys(compiled.File.Services) {
		...
		if service.PathPrefix != "" {
			if service.PathPrefix[0] != '/' || service.PathPrefix == "/" || strings.HasSuffix(service.PathPrefix, "/") || strings.Contains(service.PathPrefix, "//") {
...
func compilePolicies(compiled *Compiled) error {
	...
	for _, name := range sortedKeys(compiled.File.Policies) {
		...
		entry := CompiledPolicy{Name: name, Service: spec.Service, Revision: revision, Policy: compiledPolicy}
...
func compileRoles(compiled *Compiled) error {
	...
	for roleName, role := range compiled.File.Roles {
		if !validID(roleName) || len(role.Grants) == 0 {
...
func bindingRevision(compiled *Compiled, credentialName string, credential CredentialConfig) (string, error) {
	...
	material := struct {
		Engine                                  string
		ServiceName, Upstream, PathPrefix, Host string
...
func policyRevision(spec PolicyConfig, compiled *policy.Policy) (string, error) {
	material := struct {
		...
		Engine: policy.EngineRevision, Service: spec.Service,
...
func validHeader(auth HeaderAuth) bool {
	return validHeaderName(auth.Header) && !strings.ContainsAny(auth.Prefix, "\r\n\x00") && len(auth.Prefix) <= 256
}
